...
const findNeedle = (haystack, searchTerm) =>
{
  ...
  Object.keys(haystack).map(key =>
  {
    ...
    if (myString.includes(searchTerm.toLowerCase())) {result=true}
  }
  );
  return result;
};
...
const getWordFrequencies = str =>
{
  ...
  if (str)
  {
    ...
    wordArray.forEach(word => {if (!resultHash[word]) {resultHash[word]=1} else {resultHash[word]=resultHash[word]+1}});
  }
  return resultHash;
};
